...
public class NachosProcess implements Runnable {
	/**
	 * A static int to count the number of processes having been created/
	 */
	public static int static_pid = 0;

	/**
	 * A int represent the identity of the process/
	 */
	private int pid;
	...
	public NachosProcess(String pProcessName) {
		...
		// adding process to processes table
		JNachos.mProcesses.put(pid, this);
	}
	...
	public void finish() {
		...
		// reclaim the page from memory
		if (JNachos.getCurrentProcess().getPid() != 0) {
			//System.out.println("Process" + JNachos.getCurrentProcess().getPid() + " Clear Space size: " + MMU.mPageTable.length);
			
			for (int i = 0; i < MMU.mPageTable.length; i++) {
				// whether the page is in memory
				if (MMU.mPageTable[i].valid == true) {
					//System.out.println("Clear virtual page: " + i);
					
					// whether the page is revised, if yes, update Swap Space
					if (MMU.mPageTable[i].dirty == true) {
						//System.out.println("Update Swap Space");
						byte[] bytes = new byte[Machine.PageSize];
						System.arraycopy(Machine.mMainMemory, MMU.mPageTable[i].physicalPage * Machine.PageSize, bytes,
								0, Machine.PageSize);
						JNachos.mSwapSpace.writeAt(bytes, Machine.PageSize, MMU.mPageTable[i].SWSPPage * Machine.PageSize);
					}

					// clear bit map
					AddrSpace.mFreeMap.clear(MMU.mPageTable[i].physicalPage);
					
					// Zero out all of main memory
					//System.out.println("Clear physical page: " + MMU.mPageTable[i].physicalPage);
					Arrays.fill(Machine.mMainMemory, MMU.mPageTable[i].physicalPage * Machine.PageSize,
							(MMU.mPageTable[i].physicalPage + 1) * Machine.PageSize, (byte) 0);
					MMU.mPageTable[i].physicalPage = -1;
					MMU.mPageTable[i].valid = false;
					MMU.mPageTable[i].use = false;
					MMU.mPageTable[i].dirty = false;
				}
			}
			
			// clear out pages of this process from Page List
			int delProcess = JNachos.getCurrentProcess().getPid();
			//System.out.println();
			for(int j = 0; j < JNachos.pageRamList.size(); j++)
				if (JNachos.pageRamList.get(j)[0] == delProcess) {
					//System.out.println("Size of Page List: " + JNachos.pageRamList.size());
					//System.out.println("Clear Process" + delProcess + " page in Page List: " + JNachos.pageRamList.get(j)[1]);
					JNachos.pageRamList.remove(j);
					j--;
				}
		}
		...
		// remove process from processes table
		JNachos.mProcesses.remove(JNachos.getCurrentProcess().getPid());

		// Display current count of page faults
		//System.out.println("Current Page Faults count: " + JNachos.numOfPageFaults);
	 ...
	 *
	 **/
	public void saveUserRegister(int reg, int value) {
	...
	/**
	 * Gets the pid for this process.
	 * 
	 * @return int the pid for this thread.
	 */
	public int getPid() {
		return pid;
	}
