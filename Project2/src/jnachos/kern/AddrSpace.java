...
public class AddrSpace {
	...
	/**
	 * tool to find free space in memory
	 */
	...
	public AddrSpace(OpenFile executable) {
		...
		for (int i = 0; i < mNumPages; i++) {
			...
			if ((i * Machine.PageSize) < (noffH.code.size + noffH.initData.size)) {
				...
				/*// Zero out all of SwapSpace
				byte[] zero = new byte[Machine.PageSize];
				for (int j = 0; j < zero.length; j++) {
					zero[j] = 0;
				}
				JNachos.mSwapSpace.writeAt(zero, Machine.PageSize, mPageTable[i].SWSPPage * Machine.PageSize);*/
				...
				// Copy the buffer into the SwapSpace
				JNachos.mSwapSpace.writeAt(bytes, Machine.PageSize, mPageTable[i].SWSPPage * Machine.PageSize);
			}
		}
	}
	...
	public AddrSpace(AddrSpace pToCopy) {
		...
		for (int i = 0; i < mNumPages; i++) {
			...
			mPageTable[i].dirty = false;
			mPageTable[i].readOnly = false;
			/*mPageTable[i].valid = pToCopy.mPageTable[i].valid;
			mPageTable[i].use = pToCopy.mPageTable[i].use;
			mPageTable[i].dirty = pToCopy.mPageTable[i].dirty;
			mPageTable[i].readOnly = pToCopy.mPageTable[i].readOnly;*/
			...
			// Copy the buffer into SwapSpace
			byte[] bytes = new byte[Machine.PageSize];
			JNachos.mSwapSpace.readAt(bytes, Machine.PageSize, pToCopy.mPageTable[i].SWSPPage * Machine.PageSize);
			JNachos.mSwapSpace.writeAt(bytes, Machine.PageSize, mPageTable[i].SWSPPage * Machine.PageSize);
